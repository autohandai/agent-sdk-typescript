 ...
 * Uses raw HTTP calls to Z-AI API.
...
export class ZaiProvider implements Provider {
  ...
  constructor(config: ZaiConfig, defaultModel: string = "glm-5.1") {
  ...
  async chat(
    ...
    logger.debug('Zai chat request started', {
      model,
    ...
    try {
      const response = await withRetry(
        ...
        ),
        {
          ...
          onRetry: (attempt, error, delayMs) => {
            logger.warn(`Zai retry attempt ${attempt}`, {
              error: error.message,
              delayMs,
            });
          },
      ...
      logger.debug('Zai chat request succeeded', {
      ...
      if (error instanceof Error) {
        logger.error('Zai chat request failed', error, {
          model,
          messagesCount: messages.length,
        });
      }
